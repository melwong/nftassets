...
<div class="col-md-12">
    <div class="icon-header">
        ...
        <h2><?php printf( __( 'Booking #%s details', 'woocommerce-bookings' ), esc_html( $booking->get_id() ) ); ?></h2>
        <h3>
            <?php
            $order_id = $order ? absint( ( is_callable( array( $order, 'get_id' ) ) ? $order->get_id() : $order->id ) ) : '';
            if ( $order ) {
                /* translators: 1: href to order id */
                printf( ' ' . __( 'Linked to order %s.', 'woocommerce-bookings' ), '<a href="' . wcmp_get_vendor_dashboard_endpoint_url( get_wcmp_vendor_settings( 'wcmp_vendor_orders_endpoint', 'vendor', 'general', 'vendor-orders' ), $order_id ) . '">#' . esc_html( $order->get_order_number() ) . '</a>' );
            }
