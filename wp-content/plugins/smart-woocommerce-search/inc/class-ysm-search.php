...
class Ysm_Search
{
	...
	protected static function search_posts($s = '') {
		...
		if ( ! empty( $fields_list ) ) {
			...
			//Mel: To display only private products
			//UPDATE: 06/07/19. We no longer set each parent product to be private. Instead all parent/main products are public and all vendors' products are hidden (exclude-from-search)
			...
			if ( isset(self::$pt['product']) ) {
				...
				if ( version_compare( WC()->version, '3.0.0', '<' ) ) {
					...
					if ( $wc_product_visibility_term_ids['exclude-from-search'] ) {
						//Mel
						//UPDATE: 06/07/19. We no longer set each parent product to be private. Instead all parent/main products are public and all vendors' products are hidden (exclude-from-search)
						//To overwrite "exclude from search" option. Because we set each parent product to be private with Catalog visibility:	Hidden. So that it will not appear on the main site or in default search. But we want them to appear in this search plugin	
	...
	protected static function get_suggestions($posts) {
		...
		foreach ($posts as $post) {
			...
			//Mel: 22/06/19. To add the "?get=tickets" param
			$output = '<a href="' . esc_url( get_permalink($post->ID) ) . '?get=tickets" class="smart-search-post post-' . (int) $post->ID . '">';
