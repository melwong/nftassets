...
// This means that the image is from an existing project so we have to return a brand new image
    ...
    while (file_exists($originalImage)) {				
		preg_match('/.*(\/wp\-content\/uploads\/\d+\/\d+\/.*)/', $originalImage, $mat);				
		if(count($mat) > 0) $originalImage = ABSPATH . $mat[1];
        $i++;
    }
	...
	preg_match('/.*(\/wp\-content\/uploads\/\d+\/\d+\/.*)/', $imageURL, $mat);
	if(count($mat) > 0) $imageURL = ABSPATH . $mat[1];
