...
jQuery( function( $ ) {
  ...
  var pewc_actions = {
    ...
    /**
      ...
      $.ajax({
          ...
          // $( clone_row ).find( '.pewc_group_id' ).attr( 'name', '_product_extra_groups[group_id][' + new_group_id + ']' ).val( new_group_id );
          ...
          $( clone_row ).find( '.pewc-group-description' ).attr( 'name', '_product_extra_groups['+ new_group_id +'][meta][group_description]' );
      		$( clone_row ).find( '.pewc-group-layout' ).attr( 'name', '_product_extra_groups['+ new_group_id +'][meta][group_layout]' );
          ...
          if( response.success ) {
            ...
            $( clone_item )
              .find( '.pewc-calculation-field' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][formula]' );

            $( clone_item )
              .find( '.pewc-field-round' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][formula_round]' );

            $( clone_item )
              .find( '.pewc-decimal-places' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][decimal_places]' );

            $( clone_item )
              .find( '.pewc-field-action' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][formula_action]' );

            $( clone_item )
              .find( '.pewc-multiple-uploads' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][multiple_uploads]' );

            $( clone_item )
              .find( '.pewc-multiply-price' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][multiply_price]' );

            $( clone_item )
              .find( '.pewc-field-max-files' )
        			.attr( 'name', '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][max_files]' );
            ...
            for( var k in fields_object ) {
              $( clone_item )
          			.find( '.' + k )
          			.attr( 'name' , '_product_extra_groups[' + group_id + '][items][' + new_item_id + '][' + fields_object[k] + ']' );
            }
    ...
    /**
      ...
      $( clone_row ).attr( 'data-row-count', row_count );
  		$( clone_row )
  			...
  			.attr( 'name','_product_extra_groups[' + group_id + '][items][' + item_id + '][field_rows][' + row_count + '][label]' )
  			.val( '' );

  		$( clone_row )
  			.find( '.pewc-field-row-data' )
  			.attr( 'name','_product_extra_groups[' + group_id + '][items][' + item_id + '][field_rows][' + row_count + '][data]' )
  			.val( '' );

  		$( clone_row )
  			.find( '.pewc-image-attachment-id' )
  			.attr( 'name','_product_extra_groups[' + group_id + '][items][' + item_id + '][field_rows][' + row_count + '][image]' );
    ...
    /**
      ...
      var wrapper = $( this ).closest( '.pewc-fields-conditionals' );
  		...
  		if( r == true ) {
            ...
            var count = $( wrapper ).find( '.product-extra-conditional-rule' ).length;
  					if( count < 2 ) {
    ...
    /**
      ...
      if( $(this).hasClass( 'pewc-field-default-field-checkbox' ) ) {
        $( this ).closest( '.pewc-default-fields' ).find( '.pewc-field-default-hidden' ).val( $( this ).attr( 'checked' ) );
      } else {
  ...
  function update_conditional_fields() {
    ...
    for( var group in all_fields ) {
      ...
      for( var field in items ) {
        ...
        if( items[field].type == 'upload' || items[field].type == 'information' ) continue;
  ...
  // Return the type of value field based on the condition field selected
	function pewc_get_value_field_type( field_type ) {
		if( field_type == 'number' || field_type == 'cost' || field_type == 'calculation' || field_type == 'quantity' ) {
  ...
  function pewc_set_rules( field, has_multiple, is_cost ) {
    if( is_cost == null || field == undefined ) {
      ...
      if( is_cost.indexOf( 'cost' ) > -1 || is_cost.indexOf( 'quantity' ) > -1 || field_type == 'calculation' || field_type == 'number' ) {
