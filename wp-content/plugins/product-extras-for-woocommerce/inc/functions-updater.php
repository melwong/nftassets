...
add_action( 'woocommerce_scheduled_sales', 'pewc_daily_check_license' );
...
function pewc_do_license_activation( $license ) {
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		...
		if ( false === $license_data->success ) {
			...
			$message .= sprintf(
				__( ' Please ensure you enter the correct license key as the plugin will not work correctly without it. Your license key will be on the email you were sent with the download link or on <a target="_blank" href="%s">your account page</a>.', 'pewc' ),
