...
if( ! function_exists( 'pewc_set_upload_dir' ) ) {
	function pewc_set_upload_dir( $upload ) {
		global $woocommerce;
		$client_id = md5( $woocommerce->session->get_customer_id() );
		...
		$upload['subdir'] = '/product-extras' . $upload['subdir'] . '/' . $client_id;
...
function pewc_create_protection_files( $upload_path ) {
