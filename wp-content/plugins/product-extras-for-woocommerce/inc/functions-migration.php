...
function pewc_migration_notice() {
	...
	if( ! isset( $screen->id ) || $screen->id == 'posts_page_pewc_migration' ) {
		return;
	}
	...
	if( ! $has_migrated ) {
		...
		<div class="notice notice-error is-dismissible pewc-dismiss-migration pewc-is-dismissible-pewc-notice">
			<p>
				<strong><?php _e( 'WooCommerce Product Add-Ons Ultimate Version 3.0', 'pewc' ); ?></strong>
			</p>
			<p>
				<?php
				_e( 'From version 3.0 of Product Add-Ons Ultimate, the method used to store field data changed. In order to make best use of this change, you will need to update your database. This is important because the old method will not be supported in future versions of the plugin. Please click the button below to go to the migration page for more details.', 'pewc' ); ?>
			</p>
			<p>
...
function pewc_get_products_with_add_ons() {
	...
	if( $products ) {
		set_transient( 'pewc_migratable_products', $products->posts );
...
function pewc_do_migration() {
	...
	if( $offset > $total ) {
		...
		$products = get_transient( 'pewc_migratable_products', false );
		...
		set_transient( 'pewc_ids_map', $map );
		$offset += 10;
...
function pewc_process_globals() {
	...
	if( $global_add_ons && ! $already_done ) {
		...
		set_transient( 'pewc_global_ids_map', $map );
	}
