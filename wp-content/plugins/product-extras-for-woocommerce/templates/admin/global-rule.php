...
<div class="pewc-global-rules-wrapper">
	...
	<div class="pewc-rule-instruction-wrapper">
		...
		<span>
			<select class="pewc-rule-field" name="_product_extra_groups[<?php echo esc_attr( $group_key ); ?>][global_rules][operator]" id="_product_extra_groups[<?php echo esc_attr( $group_key ); ?>][global_rules][operator]">';
				<?php
			...
			<table class="widefat wp-list-table product-extra-global-rule-row">
				...
				<tbody id="the-list">
					<?php foreach( $all_rules as $rule ) { ?>
						...
						<tr <?php echo $class; ?>>
							<td>
								<input class="pewc-rule-field" type="checkbox" <?php echo $checked; ?> name="_product_extra_groups[<?php echo esc_attr( $group_key ); ?>][global_rules][<?php echo esc_attr( $rule['id'] ); ?>][is_selected]" id="_product_extra_groups[<?php echo esc_attr( $group_key ); ?>][global_rules][<?php echo esc_attr( $rule['id'] ); ?>][is_selected]">
							</td>
							<td>
								<label for="_product_extra_groups[<?php echo esc_attr( $group_key ); ?>][global_rules][<?php echo esc_attr( $rule['id'] ); ?>][is_selected]"><?php echo esc_html( $rule['title'] ); ?></label>
							</td>
							<td>
								<?php if( isset( $rule['callback'] ) && function_exists( $rule['callback'] ) ) {
									$select = call_user_func( $rule['callback'], $group_key, $group, $rule );
									echo $select;
								} ?>
							</td>
					<?php } ?>
