	...
	final class BE_WooCommerce_PDF_Invoices {
		...
		private function define_constants() {
			...
			if ( ! defined( 'BEWPI_INVOICES_DIR' ) ) {
				define( 'BEWPI_INVOICES_DIR', get_template_directory() . '/bewpi-invoices' );
			}
			...
			if ( ! defined( 'WPI_TEMPLATES_DIR' ) ) {
				define( 'WPI_TEMPLATES_DIR', get_template_directory() . '/woocommerce-pdf-invoices/templates' );
			}
		...
		private function init_hooks() {
			...
			add_shortcode( 'bewpi-download-invoice-multi', array( $this, 'download_invoice_shortcode_multi' ) );
			
			//Mel
			add_shortcode( 'bewpi-download-ticket', array( $this, 'download_ticket' ) );
			add_shortcode( 'bewpi-download-ticket-multi', array( $this, 'download_ticket_multi' ) );
		}
		...
		public static function setup_directories() {
			...
			$directories        = apply_filters( 'bewpi_uploads_directories', array(
				...
				//Mel
				WPI_UPLOADS_DIR . '/templates/invoice/ticket/' => array(),
			) );
		 ...
		 */
		public function download_ticket( $atts ) {
			...
			//printf( '<a target="_blank" href="%1$s">%2$s</a>', esc_attr( $url ), esc_html( $title ) );
			echo esc_attr($url);
		}
		
		/**
		 * Mel: 21/07/19
		 * Shortcode to download multiple tickets.
		 *
		 * @param array $atts shortcode attributes.
		 */
		public function download_ticket_multi( $atts ) {
			if ( ! isset( $atts['order_id'] ) || 0 === intval( $atts['order_id'] ) ) {
				return;
			}
			...
			//Mel: To add in the ticket barcode and wallet data
			$url = add_query_arg( array(
				'bewpi_action' => 'view',
				'post' => $order_id,
		...
		/**
		 * Shortcode to download invoice multi.
		 *
		 * @param array $atts shortcode attributes.
		 */
		public function download_invoice_shortcode_multi( $atts ) {
			...
			//Mel: 21/07/19. To add in the ticket barcode and wallet data
			$url = add_query_arg( array(
				'bewpi_action' => 'view',
				'post' => $order_id,
				'item_id' => $atts['item_id'],
				'nonce' => wp_create_nonce( 'view' ),
				'ticket_barcode' => $atts['ticket_barcode'],
				'address' => $atts['address'],
				'private_key' => $atts['private_key'],
				'menmonic_phrase' => $atts['mnemonic_phrase'],
			) );
/* 			$url = add_query_arg( array(
				'bewpi_action' => 'view',
				'post' => $order_id,
		...
		/**
		 * Mel
		 * Templater ticket instance.
		 *
		 * @return BEWPI_Template_Ticket.
		 */
		public function templater_ticket() {
			return BEWPI_Template_Ticket::instance();
		}
