...
$.widget( "evol.colorpicker", {
	...
	options: {
		...
		strings: ',Standard Colors,Web Colors,Theme Colors,Back to Palette,History,No history yet.'
	},
		...
		switch(this.element.get(0).tagName){
			case 'INPUT':
				...
				e.addClass('colorPicker '+this._id)
					.wrap('<div class="colorPickerCover"><div class="colorPickerWrap evo-cp-wrap" style="width:'+(this.options.hideButton?this.element.width():this.element.width()+42)+'px;'+
	...
	_paletteHTML1: function() {
		...
		var h='<table class="evo-palette'+_ie+'"><tr>';
        if (labels[0].length > 0) {
           h='<table class="evo-palette'+_ie+'">'+oTRTH+labels[0]+'</th></tr><tr>';            
        }
		...
		if (opts.customTheme) {
            ...
            if (opts.customTheme2) {
                h+='</tr><tr>';
    			for (i=0, ml=opts.customTheme2.length;i<ml;i++) {
                    h+=oTD+opts.customTheme2[i]+cTD;
                }
            }
		} else {
			oTD+='#';
