...
(function ($) {
    $.ClassyGradient = function (element, options) {
        var defaults = {
            ...
            onDeactivePoint: function () {
            }
        };
        var $element = $(element), _container, _canvas, $pointsContainer, $pointsInfos;
        var $pointsInfosContent, $pointColor, $pointPosition, $btnPointDelete, $showcopicker, $comColorPicker, _context, _selPoint;
...
//            out += 'background: ' + 'linear-gradient(' + defDir + ',' + defCss + ');';
        ...
        this.setDiscrete = function (discrete) {
            this.settings.discrete = discrete;
        ...
        this._setup = function () {
            ...
            _canvas = $('<canvas class="canvas" id="' + this.settings.id + '" width="' + this.settings.width + '" height="' + this.settings.height + '"></canvas>');
            ...
            $pointsInfos.append('<div class="arrow-up"></div>');
            _container.append($pointsInfos);
            $pointsInfosContent = $('<div class="picker-content"></div>');
            ...
            $showcopicker = $('<a href="#" class="showcopicker"><div style="background-color: #00ff00"></div></a>');
            $pointColor = $('<div class="point-color"></div>');
            $pointColor.append($showcopicker);
            $pointPosition = $('<span class="point-position">%</span>');
            $btnPointDelete = $('<a href="#" class="delete"><span class="fa fa-trash"></span> Remove Point</a>');
            $comColorPicker = $('<input id="copicker-'+self.settings.colorname+'" class="copicker" value="#ffffff"></div>');
            $pointsInfosContent.append($pointColor, $pointPosition, $comColorPicker, $btnPointDelete);
            $element.append(_container);
            var colorname = this.settings.colorname;
            $comColorPicker.colorichpicker({showOn: 'button', transparentColor: true, canvaspicker: true}).on('change.color', function (evt, color) {
                ...
                if (color.toLowerCase() == "#0000ffff")
                    color = "transparent";
                $element.find('.point-color div').css('backgroundColor', color);
                if (color == "transparent") {
                    $element.find('.evo-pointer').addClass('evo-transparent');
                } else {
                    $element.find('.evo-pointer').removeClass('evo-transparent');                    
                }
                $element.find('.point').data("transparent", (color == "transparent"));
                
                self._renderCanvas();
                self._renderToTarget();
            }).on('canvaspick.color', function (evt) {             
                WPImager.savePickerColor(colorname, $(this).val());
                $pointsInfos.hide();
            });
...
//            });
            $(document).bind('click', function () {
                if (!$element.is('.hover')) {
                    $pointsInfos.hide();
                    $comColorPicker.colorichpicker("hidePalette");
//                    if (typeof this.settings.onDeactivePoint !== "undefined") {
//                        this.settings.onDeactivePoint();
//                    }
                }
            });
        ...
        this._initGradientPoints = function () {
            ...
            $.each(points, function (i, el) {
                if (el[1].toLowerCase() == "#0000ffff") {
                    $pointsContainer.append('<div class="point evo-transparent" data-transparent="true" style="background-color: #ffffff; left:' + (parseInt(el[0]) * self.settings.width) / 100 + 'px;"></div>');
                } else {
                    $pointsContainer.append('<div class="point" data-transparent="false" style="background-color: ' + el[1] + '; left:' + (parseInt(el[0]) * self.settings.width) / 100 + 'px;"></div>');
                }
            });
        ...
        this._selectPoint = function (el) {
            ...
            var isTransparent = $(el).data("transparent");
            if (isTransparent) {
                $pointColor.find('div').css('backgroundColor', "#ffffff");
                $pointColor.find('div').addClass('evo-transparent');
            } else {
                color = color.substr(4, color.length);
                color = color.substr(0, color.length - 1);
                $pointColor.find('div').css('backgroundColor', this._rgbToHex(color.split(',')));
                $pointColor.find('div').removeClass('evo-transparent');
            }
            $pointPosition.html('Position: ' + position + '%<br>');
            ...
            WPImager.savePickerColorPoint(_selPoint.index());
//            $showcopicker.unbind('click').bind('click', function() {
//                debugger;
////                $pointsInfosContent.find('.copicker').show();
//                $pointsInfosContent.find('.colorPickerWrap').css("width","220px");
//                return false;
//            });
            $element.find('.evo-pop').hide();
            $element.find('.info').css('marginLeft', parseInt($(el).css('left')) - 30 + 'px').show('fast');
            if (isTransparent) {
                $comColorPicker.colorichpicker({color: "#0000FFFF"});                
            } else {
                var hexcolor = this._rgbToHex(color.split(','));
                $comColorPicker.colorichpicker({color: hexcolor});
            }
            $pointsInfosContent.find('.colorPickerWrap').css("width", "auto");
        };
        ...
        this._renderToCanvas = function () {
            ...
            var discrete = this.settings.discrete;
            var prevcolor = "";
            $.each(points, function (i, el) {
                if (!isNaN(parseInt(el[0])))
                {
                    ...
                    if (discrete && prevcolor.length > 0) {
                        gradient.addColorStop(parseInt(el[0]) / 100, prevcolor);
                    }
        ...
        this._rgbToHex = function (rgb) {
            ...
            if (typeof rgb[3] !== "undefined" && parseInt(rgb[3]) == 0) {
                return "#0000ffff";
            }
            function toHex(n) {
                n = parseInt(n, 10);
    ...
    $.fn.ClassyGradient = function (options) {
        return this.each(function () {
            if ($(this).data('ClassyGradient') === undefined)
            {
                ...
                $(this).data('ClassyGradient').settings.gradient = options.gradient;
                $(this).data('ClassyGradient').update();
            }
        });
    };
    $.fn.ClassyGradientSetDiscrete = function (discrete) {
        return this.each(function () {
            if ($(this).data('ClassyGradient') === undefined)
            {
            } else {
                $(this).data('ClassyGradient').settings.discrete = discrete;
                $(this).data('ClassyGradient').update();
            }
        });
