...
<tr class="item <?php echo esc_attr( $row_class ); ?>" data-order_item_id="<?php echo esc_attr( $item_id ); ?>">
    ...
    <td>
        <div class="order-item-detail">
            <p class="primary-color">
                <?php echo $product_link ? '<a href="' . esc_url( $product_link ) . '" class="wcmp-order-item-name">' . wp_kses_post( $item->get_name() ) . '</a>' : wp_kses_post( $item->get_name() ); ?>
            </p>
            ...
            <table>
                ...
                if ( $product && $product->get_sku() ) {
                    echo '<tr><th>' . esc_html__( 'SKU:', 'woocommerce' ) . '</th><td>' . esc_html( $product->get_sku() ) . '</td></tr>';
                }

                if ( $item->get_variation_id() ) {
                    echo '<tr><th>' . esc_html__( 'Variation ID:', 'woocommerce' ) . '</th><td>';
                ...
                ?>
                <?php do_action( 'wcmp_vendor_dash_before_order_itemmeta', $item_id, $item, $product ); ?>
                <?php require 'html-order-item-meta.php'; ?>
                <?php do_action( 'wcmp_vendor_dash_after_order_itemmeta', $item_id, $item, $product ); ?>
            </table>
			<!--Mel: 26/09/19. To upgrade from 3.3.4 to 3.4.3 -->
			<!--Mel: Begin-->
			<hr />
			<p></p>
			<p><?php _e('Send the token to the buyer via MetaMask', 'dc-woocommerce-multi-vendor'); ?></p>
			<p></p>
			<a href="<?php echo esc_url(wcmp_get_vendor_dashboard_endpoint_url(get_wcmp_vendor_settings('wcmp_vendor_order_wallet_endpoint', 'vendor', 'general', 'vendor-order-wallet'), $order->get_id())); ?>" ><img width="150" src="<?php echo $WCMp->plugin_url; ?>assets/images/connect-with-metamask.png"></a>
			
			<!--
			<button type="button" onclick="post_to_url('http://localhost/boothstand/wp-content/plugins/dc-woocommerce-multi-vendor/metamask-connector.php')" ><img src="<?php //echo $WCMp->plugin_url; ?>assets/images/connect-with-metamask.png"></button>
			-->     
			<!--Mel: End-->
    ...
    <?php do_action( 'wcmp_vendor_dash_order_item_values', $product, $item, absint( $item_id ) ); ?>
