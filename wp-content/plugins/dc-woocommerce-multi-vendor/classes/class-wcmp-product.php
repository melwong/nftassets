        ...
        if($has_product_map_id){
            ...
            if($mapped_products && count($mapped_products) >= 1){
                ...
                foreach ($mapped_products as $result) {
					...
					//Mel: 020719. To allow hidden vendor products to appear on product page but not those in the trash
                    if ($_product->get_catalog_visibility() == 'hidden' && $_product->get_status() != 'trash' && !is_user_wcmp_pending_vendor($other_user) && !is_user_wcmp_rejected_vendor($other_user) && $post->post_author != $result_post->post_author) {
