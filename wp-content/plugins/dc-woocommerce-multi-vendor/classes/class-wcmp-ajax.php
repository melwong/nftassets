    ...
    public function wcmp_list_a_product_by_name_or_gtin() {
        ...
        if (!empty($keyword)) {
            ...
            if (!$posts) {
                ...
                //Mel: To use a different search_products function that I modified in order to display only private products to vendors
				$ids = $data_store->search_products2($keyword, '', false);
                ...
                foreach ($ids as $id) {
                    ...
                    if ($product && $product_map_id) {
						...
						//Mel: 26/09/19. To upgrade from 3.3.4 to 3.4.3. In 3.3.4, I added the line of code below, but in 3.4.3 the line below the code of line below (below-below) has been commented. So, I decided to also comment my line out
						//Mel: 03/07/19. To display search result with only the main/parent product which should come with the smallest product_id values 
						//$first_inserted_map_pro_key = array_search(min(wp_list_pluck($results, 'product_id')), wp_list_pluck($results, 'product_id'));
                        //$first_inserted_map_pro_key = array_search(min(wp_list_pluck($results, 'ID')), wp_list_pluck($results, 'ID'));
				...
				//Mel: 26/09/19. To upgrade from 3.3.4 to 3.4.3
				//Mel: 03/07/19. By commenting out the below, I'm allowing the display search results that contain main/parent product
				/*if ($include) {
                    $ids = array_slice(array_intersect($ids, $include), 0, 10);
                } else {
                    $ids = array();
            ...
            if (count($product_objects) > 0) {
                foreach ($product_objects as $product_object) {
                    if ($product_object) {
						...
						//Mel: 26/09/19. To upgrade from 3.3.4 to 3.4.3
						//Mel: 04/07/19. To only display product that are visible, not hidden
						if ( $product_object->get_catalog_visibility() != 'hidden' ) {
