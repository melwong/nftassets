...
class WCMp_Vendor_Hooks {
    ...
    function __construct() {
		...
		//Mel
		add_action( 'wcmp_vendor_dashboard_vendor-order-wallet_endpoint', array( &$this, 'wcmp_vendor_dashboard_vendor_order_wallet_endpoint' ) );
	...
	//Mel: 26/09/19. To upgrade from 3.3.4 to 3.4.3
	/**Mel
     * To connect the vendor order details page to browser wallet. Followed wcmp_vendor_dashboard_vendor_orders_endpoint function
     * @global object $WCMp
     */
    public function wcmp_vendor_dashboard_vendor_order_wallet_endpoint() {
        global $WCMp, $wp;
        $vendor = get_current_vendor();

        $vendor_order = $wp->query_vars[get_wcmp_vendor_settings( 'wcmp_vendor_order_wallet_endpoint', 'vendor', 'general', 'vendor-order-wallet' )];
        
		if ( ! empty( $vendor_order ) ) {
            $WCMp->template->get_template( 'vendor-dashboard/vendor-orders/vendor-order-wallet.php', array( 'order_id' => $vendor_order ) );
        }
    }
