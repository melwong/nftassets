...
class WC_Product_Data_Store_CPT extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface {
	...
	*/
	...
	/**Mel
	 * Only show search results with private products to vendor. So that they could add their own product and after they submitted their product, their product remains in private status. Thus, admin should approve their product and then uncheck the private status. 
	 *
	 ...
	 * @return array of ids
	 */
	public function search_products2( $term, $type = '', $include_variations = false, $all_statuses = false, $limit = null, $include = null, $exclude = null ) {
		global $wpdb;
		...
		//$post_statuses = current_user_can( 'edit_private_products' ) ? array( 'private', 'publish' ) : array( 'private', 'publish' );
		$type_where    = '';
		$status_where  = '';
		$limit_query   = '';
		...
		//Mel: 03/07/19. SQL not working yet
		// phpcs:ignore WordPress.VIP.DirectDatabaseQuery.DirectQuery
/* 		$search_results = $wpdb->get_results(
			// phpcs:disable
			"SELECT DISTINCT posts.ID as product_id, posts.post_parent as parent_id 
			 FROM {$wpdb->posts} posts JOIN 
			 {$wpdb->term_relationships} tr 
			 ON posts.ID = tr.object_id JOIN 
			 {$wpdb->terms} t 
			 ON tr.term_taxonomy_id = tr.term_id 
			 LEFT JOIN {$wpdb->wc_product_meta_lookup} wc_product_meta_lookup ON posts.ID = wc_product_meta_lookup.product_id
			WHERE t.name <> 'exclude-from-catalog' AND posts.post_type IN ('" . implode( "','", $post_types ) . "')
			$search_where
			$status_where
			$type_where
