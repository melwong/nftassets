...
add_filter('woocommerce_product_cf_meta_data', function($output){
    return '';
});
...
function campaign_listing_by_author_before_loop(){
    if (! empty($_GET['author'])) {
        echo '<h3>'.__('Campaigns by: ', 'patrios').' '.wpneo_crowdfunding_get_author_name_by_login(sanitize_text_field(trim($_GET['author']))).'</h3>';
    }
