<?php
    ...
    $subtext = '';
    global $post;
    if(!function_exists('patrios_call_sub_header')){
        function patrios_call_sub_header(){
                ...
                if(!empty($image_attached)){
                    ...
                    if(empty($sub_img[0])){
                        ...
                        if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true ) == ''){
                            $output = patrios_call_sub_header();
                        }
                    }
                }else{
            ...
            <div class="col-12 text-center">
                    ...
                    if(isset($wp_query->queried_object->name)){
                        if (get_theme_mod( 'header_title_enable', true )) {
                            if($wp_query->queried_object->name != ''){
                                if($wp_query->queried_object->name == 'product' ){
                                    echo '<h2>'.esc_html__('Shop','patrios').'</h2>';
                                }else{
                                    ...
                                    if($category) {
                                        ...
                                        if ($option_name) {
                                            echo '<span class="patrios-icons"><i class="back-'.get_option($option_name).'"></i></span>';
                                        }
                        ...
                        if( is_search() ){
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                $first_char = esc_html__('Search','patrios');
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if (get_post_type() == 'event') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Event Details','patrios' ).'</h2>';
                                } elseif (get_post_type() == 'album') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Albums','patrios' ).'</h2>';
                                } elseif (get_post_type() == 'gallery') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Gallery','patrios' ).'</h2>';
                                } elseif (get_post_type() == 'performer') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Performer','patrios' ).'</h2>';
