...
<?php foreach( $item['child_products'] as $child_product_id ) {
	...
	//Mel: 23/08/19. To prevent the display of long description in child product
	$excerpt = apply_filters('the_excerpt', get_post_field('post_excerpt', $child_product_id));
/* 	$excerpt = sprintf(
	...
	if( $child_product->get_type() == 'variable' ) {
		...
		if( $variants ) {
			...
			foreach( $variants as $variant_id ) {
				...
				//Mel: 28/08/19. To get a more descriptive name for each variation. Like "Row: A, Seat: 1" instead of "A-1"
				$variation_name = get_the_excerpt( $variant_id ); 
  ...
  $name = sprintf(
	...
	//Mel: 23/08/19. To remove the placeholder image if there's no actual image uploaded by adding style="display: none;"
	if ( $image_url == wc_placeholder_img_src() ) {
		
		$checkbox = sprintf(
		'<div class="%s" data-manage-stock="%s"><label for="%s" style="display: none;"><input data-option-cost="%s" type="checkbox" name="%s[]" id="%s" class="pewc-checkbox-form-field pewc-column-form-field" value="%s" %s %s>%s</label><div class="pewc-checkbox-desc-wrapper">%s%s%s<div class="pewc-column-variants-wrapper">%s</div>%s<p class="pewc-column-add-wrapper">%s</p></div></div>',
