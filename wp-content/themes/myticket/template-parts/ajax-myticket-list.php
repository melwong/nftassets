    ...
    if ( $products->have_posts() ) : ?>
        ...
        <?php while ( $products->have_posts() ) : $products->the_post(); $meta = get_post_meta( get_the_ID() ); $i++; ?>
			...
			<!--Mel: 20/08/19. Added when upgraded to latest 1.0.7 theme. To get the min price for each simple and variable product
			<?php
            ...
            <div class="search-result-item <?php if ( strlen( $meta['_sale_price'][0] ) > 0 ){ echo 'sale '; } if ( $meta['_stock_status'][0] == 'outofstock' ){ echo 'sold-out '; } ?>">
                ...
                <div class="row">
                    ...
                    <div class="search-result-item-price col-sm-3">
                        ...
                        <?php else: ?>
							<!--Mel: 20/08/19. Added when upgraded to latest 1.0.7 theme. Modify "Price From" to show min price from variable and simple product-->
                            <strong><?php echo esc_html( $currency_symbol.$price ); //Mel: 20/08/19 echo esc_html( $currency_symbol.$_regular_price ); ?></strong>
                        <?php endif; ?>
